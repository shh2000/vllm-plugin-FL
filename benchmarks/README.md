To use the bench_perf feature from vllm-plugin-fl, you must first complete the following preliminary steps:

1. Start an LLM inference service compliant with the OpenAI API protocol using the --served-model-name Qwen3-Next argument, or use a different name and update the string on line 11 of bench_perf.py to match your chosen --served-model-name exactly (character-for-character).

2. Run the bench_perf script on the same machine where the inference service is hosted.
...
4. The bench_perf feature has been validated in environments using vLLM versions 0.11.0 and 0.12.0. Using higher or lower vLLM versions—or serving the model with alternative frameworks such as SGLang—may result in compatibility issues.
